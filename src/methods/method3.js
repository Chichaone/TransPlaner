...
const method3 = {
  id: 'pendulum-equal-loaded',
  name: '3. Маятниковый маршрут с полностью гружёным обратным пробегом (γ₁ = γ₂)',
  description:
    'Методика описывает маятниковый маршрут с полностью гружёным пробегом в обоих направлениях без холостых участков.',
  inputs: [
  ...
  calculate: (values) => {
    ...
    return {
      ...
      'Время оборота (tₒ = tₑ₁ + tₑ₂), ч': formatNumber(cycleTime),
      'Выработка за любую ездку (Qₑ), т': formatNumber(tonnagePerTrip),
      'Выработка за оборот (Qₒ), т': formatNumber(tonnagePerCycle),
      'Тонно-километры первой ездки (Pₑ₁), т·км': formatNumber(tonKmFirstTrip),
      'Тонно-километры второй ездки (Pₑ₂), т·км': formatNumber(tonKmSecondTrip),
      'Тонно-километры за оборот (Pₒ), т·км': formatNumber(tonKmPerCycle),
      ...
      'Тонно-километры за смену (Pₙ), т·км': formatNumber(tonKilometres),
