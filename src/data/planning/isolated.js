...
const calculateSingleVehicle = ({
  ...
  const tonKmPerTrip = tonnagePerTrip * loadedDistance;
  ...
  const totalDistance = Math.max(totalDistanceBase - (actualTrips > 0 ? emptyDistance : 0), 0);
...
const calculatePlan = () => {
  ...
  const planRows = requests.map((request) => {
    ...
    const base = calculateSingleVehicle({
      payload: vehicleConfig.payload,
      loadFactor: vehicleConfig.loadFactor,
      shiftDuration: request.workTime,
      serviceTime,
      loadedDistance: distances.loadedDistance,
      zeroRun1: distances.zeroRun1,
      zeroRun2: distances.zeroRun2,
      emptyDistance: distances.emptyDistance,
    ...
    const plannedDutyTime = base.actualShiftTime * vehiclesNeeded;
    ...
    return {
      ...
      perVehicleTonnage: base.totalTonnage,
      plannedTonnage,
      plannedTonKm,
      plannedDistance,
      plannedDutyTime,
  ...
  return {
    ...
    planRows: planRows.map((row) => ({
      ...
      perVehicleTonnageLabel: formatNumber(row.perVehicleTonnage),
    })),
    totals: {
